...
class MomList extends Component
{
    ...
    {
        $userQuery = Mom::query()->with('user');
        if ($this->searchName)
            $userQuery->orWhere('mom_name', 'like', '%' . $this->searchUserName . '%');            
        if ($this->searchUser) {
        ...
        if ($this->searchNumber)
            $userQuery->orWhere('mom_number', 'like', '%' . $this->searchNumber . '%');
        if ($this->searchStatus >= 0)
            $userQuery->orWhere('active', $this->searchStatus);
        return view('livewire.admin.mom.mom-list', [
            'datas' => $userQuery
    ...
    {
        Mom::destroy($id);
        $this->showModal('success', 'Success', 'Mom has been deleted successfully');
    }
    ...
    public function changeStatus(Mom $user)
    {
        $user->fill(['active' => ($user->active == 1) ? 0 : 1])->save();
        
        $this->showModal('success', 'Success', 'Mom status has been changed successfully');
    }
}
